...
void *hw_malloc(size_t bytes)
{
	...
	if(first_call){
		...
		puts("before call first_split"); fflush(stdout);
		return first_split((struct chunk_header*)(HEAP->start_brk), DEFAULT_HEAP_SIZE, bytes);
	}
	return NULL;
}
...
void heap_init(struct heap_t* heap, size_t bytes)
{
	...
	printf("size of heap = %lu, %llu\n", heap->size, (ull)sbrk(0) - (ull)heap->start_brk);
}
...
void split_upper_chunk_init(struct chunk_header* chunk, struct chunk_header *prev_chunk, size_t bytes)
{
...
void* first_split(struct chunk_header* split_chunk, size_t total_size, size_t split_size)
{
	puts("0"); fflush(stdout);
	struct chunk_header* upper_chunk = (struct chunk_header*)((ull)split_chunk + split_size);
